...
import { updateVoteItem } from '../utils/fetcher';
import useVote from '../hooks/useVote';
import jwtDecode from 'jwt-decode';
import { STATE_STARTED, STATE_ENDED, STATE_LOADING } from '../utils/customState';
...
export default function EditVotePage() {
  ...
  useEffect(() => {
    if (data) {
      ...
      if (currentState === STATE_STARTED || currentState === STATE_ENDED) {
  ...
  useEffect(() => {
    if (data && data.payload) {
      ...
      setTitle(d.title);
      setStartTime(new Date(d.startTime));
      setEndTime(new Date(d.endTime));
  ...
  const updateVote = async (e) => {
    ...
    updateVoteItem(user, title, startTime, endTime, candidates, code).then(() => {
      navigate('/dashboard')
      MySwal.fire({
        icon: 'success',
        title: 'Update Vote Successfully',
  ...
  return (
    <div className='flex flex-col space-y-6 2xl:space-y-7 p-10'>
      <h1 className='font-semibold text-2xl font-lato tracking-wider'>Update Vote</h1>

      <form className='flex flex-col space-y-10 2xl:w-[54%] w-[65%]' onSubmit={updateVote}>
        ...
        <button className='bg-[#7C5026] p-2 rounded-md w-1/6 text-white font-lato self-end mr-7' type='submit'>Update</button>
      </form>
    </div>
  )
}
